...
const ManageUsers = ({ role, title, description, classId, onBack }) => {
    ...
    const loadUsers = async () => {
        setLoading(true);
        try {
            // classId থাকলে API রুটে যুক্ত হবে
            const url = `/users?role=${role}${classId ? `&classId=${classId}` : ''}`;
    ...
    const handleOpenModal = (user = null) => {
        ...
        if (user) {
            ...
            setFormData({ name: user.name, email: user.email, role: user.role, classId: classId, password: '' });
        } else {
            setIsEditing(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                ...
                await apiClient.put(`/users/${selectedUser.id}`, { name, email, role: formData.role, classId });
            } else {
